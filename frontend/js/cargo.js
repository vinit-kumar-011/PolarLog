(function () {
  ...
  // Reverse of the *_LABELS maps above, for turning what the modal's
  // selects show (display labels) back into the lowercase enum values
  // the backend/DB expects.
  ...
  function renderTable() {
    ...
    $("#tableBody").innerHTML =
      ...
      <tr data-id="${d.id}" class="${d.id === selectedId ? "selected" : ""}">
        ...
        <td>${d.item}${d._pending ? `<span class="pending-badge${d._pendingDelete ? " delete" : ""}">${d._pendingDelete ? "PENDING DELETE" : "PENDING SYNC"}</span>` : ""}</td>
        <td>${d.category}</td>
        <td>${d.origin}</td>
        <td>${d.destination}</td>
        <td>${d.qty}</td>
  ...
  function handleRowAction(act, id) {
    ...
    if (act === "view") {
      ...
      if (row._pending) {
        toast("Wait for this cargo to sync before editing it.", "warn");
        return;
      }
      openCargoModal(row);
    } else if (act === "delete") {
      if (row._pending) {
        toast("Wait for this cargo to sync before deleting it.", "warn");
        return;
      }
  ...
  /* =========================================================
     DELETE CARGO — persists to DELETE /api/cargo/<id>.
     Offline: apiSend() queues it; the row stays visible but flagged
     _pending (locking further edit/delete) with a "PENDING DELETE"
     badge instead of "PENDING SYNC", and drops out on the next
     reload once sync-manager.js replays the delete.
  ========================================================= */
  async function deleteCargo(row) {
    const cargoId = row.id.replace("CRG-", "");
    const result = await apiSend(`/api/cargo/${cargoId}`, "DELETE");

    if (result.ok) {
      const idx = DATA.findIndex((d) => d.id === row.id);
      if (idx > -1) DATA.splice(idx, 1);
      if (selectedId === row.id) selectedId = DATA[0]?.id;
      applyFilters();
      renderStats();
      renderCargoDetails();
      toast(row.id + " deleted.", "warn");
      return;
    }
    ...
    if (result.queued) {
      ...
      renderCargoDetails();
      toast(
        "Offline — delete for " + row.id + " queued, will sync automatically.",
        "warn",
      );
      return;
    }

    toast(result.error || "Could not delete cargo.", "err");
  }
  ...
  function renderCargoDetails() {
    ...
    $("#cargoDetailsPanel").innerHTML = `
      ...
      <div class="cargo-detail-head">
        ...
        <div><div class="cargo-detail-id">${d.id}</div><div class="cargo-detail-item">${d.item}${d._pending ? `<span class="pending-badge${d._pendingDelete ? " delete" : ""}">${d._pendingDelete ? "PENDING DELETE" : "PENDING SYNC"}</span>` : ""}</div></div>
      </div>
      ...
      ${
        isEdit
          ? `<div class="form-row"><label>Status</label>
               <input value="${existing.status}" disabled>
               <p class="sub" style="margin:4px 0 0;">Use "Update Status" on the details panel to move this cargo along — Pending → In Transit → Delivered.</p>
             </div>`
          : `<div class="form-row"><label>Status</label>
               <input value="Pending" disabled>
               <p class="sub" style="margin:4px 0 0;">New cargo always starts Pending, until it's physically dispatched.</p>
             </div>`
      }
  ...
  /* =========================================================
     EDIT CARGO — persists to PUT /api/cargo/<id>. Status isn't
     editable here (the modal shows it read-only) - status only moves
     via the "Update Status" action, so it's always sent unchanged.
     Origin/destination/eta live on the linked shipment, not cargo
     itself, and the backend PUT updates both in one call.
     Offline: apiSend() queues it; changes are applied optimistically
     and the row is flagged _pending (same badge/lock as a queued
     create) until sync-manager.js replays it and a reload confirms.
  ========================================================= */
  async function submitEditCargo(existing) {
    const item = $("#fItem").value.trim() || "Untitled Cargo";
    const categoryLabel = $("#fCategory").value,
      priorityLabel = $("#fPriority").value,
      origin = $("#fOrigin").value;
    const destination = $("#fDestination").value;
    ...
    const payload = {
      item_name: item,
      category: CATEGORY_VALUES[categoryLabel] || "other",
      priority: PRIORITY_VALUES[priorityLabel] || "normal",
      quantity: qty,
      weight_kg: weight,
      unit,
      origin,
    ...
    const cargoId = existing.id.replace("CRG-", "");
    const result = await apiSend(`/api/cargo/${cargoId}`, "PUT", payload);
    ...
    if (result.queued) {
      ...
      closeModal();
      toast("Offline — changes to " + existing.id + " queued, will sync automatically.", "warn");
      renderStats();
      applyFilters();
      renderCargoDetails();
      return;
    ...
    toast(result.error || "Could not update cargo.", "err");
  }

  /* =========================================================
     NEW CARGO — works offline
     On submit: tries the network immediately via POST /api/cargo.
     The backend auto-creates a shipment to carry this cargo (using
     origin/destination/eta below) unless a shipment_id is passed, so
     origin/destination need to be sent even though cargo itself has
     no such columns.
     Online: on success, re-pulls cargo from the server so the row
     shown carries the real cargo_id/shipment link (see reloadCargo()).
     Offline: apiSend() queues it and this adds an optimistic
     "pending sync" row so it's visible right away; sync-manager.js
     replays it automatically once back online, and the pending tag
     clears on the next reload once the real row is fetched.
  ========================================================= */
  async function submitNewCargo() {
    const item = $("#fItem").value.trim() || "Untitled Cargo";
    const categoryLabel = $("#fCategory").value;
    const priorityLabel = $("#fPriority").value;
    ...
    const qty = Math.max(1, parseInt($("#fQty").value) || 1);
    const weight = Math.max(1, parseInt($("#fWeight").value) || 1);
    // New cargo always starts life as "Pending" - it hasn't physically
    // moved yet. It only becomes "In Transit"/"Delivered" via the
    // "Update Status" action once it's actually dispatched.
    ...
    const payload = {
      ...
      category: CATEGORY_VALUES[categoryLabel] || "other",
      priority: PRIORITY_VALUES[priorityLabel] || "normal",
      status: STATUS_VALUES[statusLabel] || "pending",
      quantity: qty,
      weight_kg: weight,
      unit,
      origin,
    ...
    const result = await apiSend("/api/cargo", "POST", payload);
    ...
    if (result.ok) {
      ...
      toast(
        "Cargo created" + (ref ? ` — logged as ${ref}.` : "."),
        "success",
      );
      await reloadCargo();
      return;
    ...
    if (result.queued) {
      closeModal();
      const tempId = `CRG-pending-${result.id}`;
      DATA.unshift({
        id: tempId,
        item,
        category: categoryLabel,
      ...
      selectedId = tempId;
      toast("Offline — cargo queued, will sync automatically.", "warn");
      renderStats();
      applyFilters();
      renderCargoDetails();
      return;
    ...
    toast(result.error || "Could not add cargo.", "err");
  }
  function closeModal() {
  ...
  $("#updateStatusBtn").addEventListener("click", async () => {
    ...
    if (!d) return;
    if (d._pending) {
      toast("Wait for this cargo to sync before updating its status.", "warn");
      return;
    }
    if (d.status === "Delivered") {
      toast(d.id + " has already been delivered.", "warn");
      return;
    }
    const order = ["Pending", "In Transit", "Delivered"];
    const next = order[(order.indexOf(d.status) + 1) % order.length];
    const cargoId = d.id.replace("CRG-", "");

    const result = await apiSend(`/api/cargo/${cargoId}`, "PUT", {
      status: STATUS_VALUES[next] || "pending",
    });

    if (result.ok) {
      toast(d.id + " status updated to " + next + ".", "success");
      await reloadCargo(d.id);
      return;
    }
    ...
    if (result.queued) {
      ...
      renderStats();
      toast(
        "Offline — status change for " + d.id + " queued, will sync automatically.",
        "warn",
      );
      return;
    }

    toast(result.error || "Could not update status.", "err");
  });
  ...
  // Re-pulls cargo from the real API and rebuilds DATA from scratch.
  // Used after a successful create so the row shown carries the real
  // cargo_id/shipment link the server assigned, instead of trusting a
  // client-fabricated one that would vanish (or drift) on next reload.
  async function reloadCargo(preferId) {
    const rows = await fetchCargoData();
    DATA.length = 0;
    DATA.push(...rows);
    const keep = preferId || selectedId;
    selectedId = DATA.find((d) => d.id === keep)
      ? keep
      : DATA[0]
        ? DATA[0].id
        : null;
